...
describe Person do
  ...
  describe "can create an Account" do
    ...
    it "is expected to have  Account class " do
      #之前我们创建了Account class在别的文件夹里这里表示我们现在创建的账户属于我们之前创建的Accountclass里的一个实例
      expect(subject.account).to be_an_instance_of Account
    end

    it "is expected to can  with herself as an owner" do
      #意思是我们账户持有人就是我们上面创建的subject里的那个人
      expect(subject.account.owner).to be subject
    end
  end

  describe "is expected to can manage funds if an account been created" do
    #当你创建了一个账户后你可以管理你的钱了
    let(:atm) { Atm.new }
    before { subject.create_account }
    #首先你成功建立了账户

    it "is expected to can deposit funds" do
      #之后就可以开始存钱了
      expect(subject.deposit(100)).to be_truthy
      # 你存进去一百的这个事实被接受了（存100是真的）
    end

    it "is expected to have funds are added to the account balance - deducted from cash" do #资金被添加到账户余额 - 从现金中扣除
      subject.cash = 100
      subject.deposit(100)
  ...
  describe "is expected to can not manage funds if no account been created" do
    #如果没有成功创见账户

    it 'is expected to can\'t deposit funds' do
